...
export function createRestApp(): Hono {
  ...
  app.get('/agents', async (c) => {
    try {
      ...
      const running = (data.agents as Array<{ status: string }>).filter((a) => a.status === 'running');
  ...
  app.post('/tasks', async (c) => {
    ...
    if (!title || !assigneeIds || assigneeIds.length === 0) {
      return c.json({ error: 'title and at least one assigneeId are required' }, 400);
    }
  ...
  app.patch('/tasks/:id/status', async (c) => {
    ...
    if (action === 'approve') {
      ...
      addComment(taskId, 'human', 'human', 'Human', '[System] Task approved and marked as done.');
    } else {
      if (!feedback) return c.json({ error: 'feedback is required when rejecting' }, 400);
